...
(function () {
  ...
  function filterPortfolio(tab) {
    ...
    const section = tabList && tabList.closest("section");
    const grid = section && section.querySelector("[data-portfolio-grid]");
    const mosaic = section && section.querySelector("[data-portfolio-mosaic]");
    if (!tabList || !grid) return false;

    const term = tab.getAttribute("data-portfolio-term") || "";
    ...
    grid.querySelectorAll("[data-portfolio-terms]").forEach(function (card) {
      const terms = (card.getAttribute("data-portfolio-terms") || "").split(/\s+/);
  ...
  function bindOnce() {
    ...
    document.addEventListener("click", function (e) {
      const t = e.target.closest && e.target.closest("[data-nh-menu-toggle]");
      if (t) {
        const nav = document.querySelector("[data-nh-nav]");
        if (!nav) return;
        e.preventDefault();
        t.setAttribute("aria-expanded", String(nav.classList.toggle("is-open")));
        return;
      }
      const portfolioTab = e.target.closest && e.target.closest("[data-portfolio-term]");
      if (portfolioTab) {
      ...
      const toc = e.target.closest && e.target.closest("[data-nh-toc-toggle]");
      if (toc) {
        const box = toc.closest(".article__toc");
        const list = box && box.querySelector("[data-nh-toc]");
        if (!list) return;
        const hidden = list.hasAttribute("hidden");
        if (hidden) list.removeAttribute("hidden");
        else list.setAttribute("hidden", "");
        toc.textContent = hidden ? "[Ẩn]" : "[Hiện]";
        toc.setAttribute("aria-expanded", String(hidden));
        return;
      }
      const vid = e.target.closest && e.target.closest("[data-nh-video]");
      if (vid) {
        e.preventDefault();
        openVideo(vid.getAttribute("data-nh-video") || vid.getAttribute("href"));
        return;
      }
      const slideBtn = e.target.closest && e.target.closest("[data-nh-slider-prev],[data-nh-slider-next]");
      if (slideBtn && !slideBtn.disabled) {
        const root = slideBtn.closest("[data-nh-slider]");
        const track = root && root.querySelector("[data-nh-slider-track]");
        if (!track) return;
        e.preventDefault();
        const card = track.querySelector(".nh-team-card");
    ...
    const read = function () {
      ...
      requestAnimationFrame(function () {
        ticking = false;
        const header = document.querySelector("[data-nh-header]");
        if (!header) return;
        const y = window.scrollY;
        const nav = document.querySelector("[data-nh-nav]");
        const menuOpen = nav && nav.classList.contains("is-open");
        const searchOpen = document.body.classList.contains("nh-search-locked");
        const videoOpen = document.body.classList.contains("nh-video-locked");
  ...
  function initSliders() {
    document.querySelectorAll("[data-nh-slider]").forEach(function (root) {
      ...
      const track = root.querySelector("[data-nh-slider-track]");
      const prev = root.querySelector("[data-nh-slider-prev]");
      const next = root.querySelector("[data-nh-slider-next]");
      if (!track) return;
  ...
  function youtubeId(value) {
    ...
    const m = String(value).match(/(?:youtu\.be\/|v=|embed\/)([\w-]{11})/);
    if (m) return m[1];
  ...
  function initVideo() {
    ...
    root.innerHTML =
      ...
      '<div class="nh-video-modal__dialog">' +
        '<button class="nh-video-modal__close" type="button" data-nh-video-close aria-label="Đóng video">Esc</button>' +
    ...
    function open(id) {
      ...
      const closeBtn = root.querySelector("[data-nh-video-close]");
  ...
  function collect() {
    ...
    const out = [];
    const seen = {};
    const add = function (title, kind, url) {
  ...
  function mark(title, q) {
    const i = fold(title).indexOf(fold(q));
  ...
  function initSearch() {
    ...
    document.addEventListener("click", function (e) {
      const t = e.target.closest && e.target.closest(".nh-header__search,[data-nh-search-open]");
      if (!t) return;
      e.preventDefault();
      buildSearch().open();
    });
  ...
  function buildSearch() {
    ...
    const headerSearchIcon = document.querySelector(".nh-header__search .nh-icon-mask");
    const searchIconStyle = headerSearchIcon ? ' style="' + headerSearchIcon.getAttribute("style") + '"' : "";
    root.innerHTML =
      ...
      '<div class="nh-search__dialog">' +
        '<label class="nh-search__field">' +
          ...
          '<button class="nh-search__close" type="button" data-nh-search-close aria-label="Đóng tìm kiếm">Esc</button>' +
        "</label>" +
        '<p class="nh-search__group" data-nh-search-group>Gợi ý</p>' +
        '<ul class="nh-search__list" data-nh-search-list role="listbox" aria-live="polite"></ul>' +
        '<div class="nh-search__hint"><span><kbd>↑↓</kbd>chọn</span><span><kbd>↵</kbd>mở</span><span><kbd>Esc</kbd>đóng</span></div>' +
      "</div>";
    document.body.appendChild(root);

    const input = root.querySelector(".nh-search__input");
    const list = root.querySelector("[data-nh-search-list]");
    const group = root.querySelector("[data-nh-search-group]");
    let index = [];
    let rows = [];
    let active = -1;
    let last = null;
    ...
    function render() {
      ...
      list.innerHTML = rows.map(function (r, i) {
        return '<li role="presentation"><a class="nh-search__item' + (i === 0 ? " is-active" : "") + '" href="' + escapeHtml(r.url) + '" role="option" id="nh-search-opt-' + i + '" aria-selected="' + (i === 0 ? "true" : "false") + '"><span>' + mark(r.title, q) + '</span><span class="nh-search__kind">' + escapeHtml(r.kind) + "</span></a></li>";
      }).join("");
      input.setAttribute("aria-activedescendant", "nh-search-opt-0");
    }

    function setActive(next) {
      const items = list.querySelectorAll(".nh-search__item");
    ...
    root.addEventListener("keydown", function (e) {
      if (e.key === "Escape") { e.preventDefault(); close(); }
        ...
        const el = list.querySelectorAll(".nh-search__item")[active];
