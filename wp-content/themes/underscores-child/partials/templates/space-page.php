...
$render_related = static function (WP_Query $query): void {
    ...
    <section class="section-posts"><div class="nh-container nh-stack"><div class="nh-rule-head">
        ...
        <a class="nh-rule-head__action" href="<?php echo esc_url(get_post_type_archive_link('post') ?: home_url('/')); ?>"><span><?php esc_html_e('Tất cả bài viết', 'underscores-child'); ?></span><?php echo underscores_child_icon_mask('icon_arrow_right'); ?></a>
...
if ($section_enabled($categories_settings) && $categories !== []) : ?>
    <section class="section-panel"><div class="nh-container"><div class="nh-category-panel">
        <?php foreach ($categories as $category) : if (!is_array($category)) { continue; } $href = is_array($category['link'] ?? null) ? (string) ($category['link']['url'] ?? '') : (string) ($category['url'] ?? ''); $icon_id = absint($category['icon'] ?? 0); if ($href === '') { continue; } ?>
            <a class="nh-category" href="<?php echo esc_url($href); ?>">
                <?php if ($icon_id > 0) : ?><?php echo wp_get_attachment_image($icon_id, 'thumbnail', false, ['class' => 'nh-category__icon', 'alt' => '', 'loading' => 'lazy']); ?><?php endif; ?>
...
if ($section_enabled($projects_settings) && $projects->have_posts()) : ?>
    ...
    <section class="section-projects"><div class="nh-container nh-stack">
        <div class="nh-section-head"><div><?php if (!empty($projects_settings['eyebrow'])) : ?><p class="nh-eyebrow"><?php echo esc_html((string) $projects_settings['eyebrow']); ?></p><?php endif; ?><?php if (!empty($projects_settings['title'])) : ?><h2 class="nh-section-title"><?php echo esc_html((string) $projects_settings['title']); ?></h2><?php endif; ?></div><?php if (!empty($projects_settings['link'])) : ?><?php echo underscores_child_acf_link($projects_settings['link'], '<span>' . esc_html__('Xem tất cả', 'underscores-child') . '</span><span class="nh-cta__arrow" aria-hidden="true">' . underscores_child_icon_mask('icon_arrow_right') . '</span>', 'nh-cta nh-cta--ink'); ?><?php endif; ?></div>
        <div class="nh-grid-4">
...
if ($section_enabled($values_settings) && ($value_image !== '' || !empty($values_settings['title']) || $value_steps !== [])) : ?>
    <section class="nh-band"><div class="nh-band__image"><?php if ($value_image !== '') : ?><img src="<?php echo esc_url($value_image); ?>" alt="" loading="lazy"><?php endif; ?></div><div class="nh-band__content nh-container"><div class="nh-values-grid">
        ...
        <?php if ($value_steps !== []) : ?><div class="nh-process-canvas"><svg class="nh-process-path" viewBox="0 0 697 381" fill="none" aria-hidden="true"><path d="M75.5 24H656A41 41 0 0 1 697 65V189A41 41 0 0 1 656 230H181.5"/></svg><?php foreach ($value_steps as $index => $step) : if (!is_array($step)) { continue; } $step_number = $index + 1; $step_icon_id = absint($step['icon'] ?? 0); ?><div class="nh-process-step nh-process-step--<?php echo esc_attr((string) $step_number); ?>"><span class="nh-process-step__number"><?php echo esc_html(sprintf('%02d', $step_number)); ?></span><span class="nh-process-step__body"><?php if ($step_icon_id > 0) : ?><?php echo wp_get_attachment_image($step_icon_id, 'thumbnail', false, ['class' => 'nh-process-step__icon', 'alt' => '', 'loading' => 'lazy']); ?><?php endif; ?><?php if (!empty($step['title'])) : ?><strong><?php echo esc_html((string) $step['title']); ?></strong><?php endif; ?><?php if (!empty($step['text'])) : ?><small><?php echo wp_kses_post((string) $step['text']); ?></small><?php endif; ?></span></div><?php endforeach; ?></div><?php endif; ?>
    </div></div></section>
<?php endif;
